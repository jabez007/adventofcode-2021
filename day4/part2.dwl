...
fun loseBingo(boards: Array<Board>, drawNumbers: Array<Number>, winningScores = []): Number =
    if (isEmpty(drawNumbers))
        winningScores[-1] default 0
    else do {
        var calledNumber = drawNumbers[0]
        var markedBoards = boards map markBoard($, calledNumber)
        var wonBoard = markedBoards dw::core::Arrays::firstWith checkBoard($)
        ...
        if (wonBoard != null)
            loseBingo(
                markedBoards filter not checkBoard($),
                drawNumbers dw::core::Arrays::drop 1,
                winningScores ++ [calledNumber * sum(flatten(wonBoard.rows))]
            )
        else
            loseBingo(markedBoards, drawNumbers dw::core::Arrays::drop 1, winningScores)
    }

output application/json
---
loseBingo(boards, drawNumbers)
